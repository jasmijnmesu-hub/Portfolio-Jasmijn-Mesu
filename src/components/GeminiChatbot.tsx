    ...
    <div className="fixed bottom-5 right-5 z-50">
      {isOpen && (
        <section className="mb-3 flex h-[min(32rem,calc(100vh-7rem))] w-[min(22rem,calc(100vw-2rem))] flex-col border border-[#1B2A24]/15 bg-[#EDE6D8] shadow-2xl" aria-label="Portfolio chatbot">
          <div className="flex items-center justify-between border-b border-[#1B2A24]/10 bg-[#F3EDE5] p-4">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto p-4">
            ...
            {messages.map((message, index) => (
              <div key={`${message.role}-${index}`} className={`max-w-[90%] p-3 text-xs leading-relaxed ${message.role === 'user' ? 'ml-auto bg-[#9C4A32] text-[#EDE6D8]' : 'bg-[#F3EDE5]'}`}>
                {message.text}
              </div>
            ))}
